...
vec3 fade(vec3 t) {
  return vec3(1.0,1.0,5.0);//t*t*t*(t*(t*6.0-15.0)+10.0);
}
...
void main( void ) {
	...
	for (float i=1.0; i<=390.0; i++)
	{
		vec2 star_pos = vec2(sin(i) * 250.0, cos(i*i*i/time) * 250.0);
		//float z = mod(i*i - starspeed*time, 256.0);
		float fade = (256.0 - z) /256.0;
		vec2 blob_coord = star_pos / z;
		gradient += ((fade / 384.0) / pow(length(centered_coord - blob_coord), 1.618)) * ( fade);
	}
